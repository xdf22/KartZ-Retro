...
static void P_ClearSingleMapHeaderInfo(INT16 i)
{
	...
	DEH_WriteUndoline("SKYMOUNT", va("%d", mapheaderinfo[num].skymount), UNDO_NONE);
	mapheaderinfo[num].skymount = 0;
#endif
}
...
static void P_LevelInitStuff(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].health = 1;
#ifdef RIDERS
		memset(players[i].checkpointtimes, 0, sizeof(players[i].checkpointtimes));
...
#ifdef RIDERS
	if (kartmode && leveltime < 157)
		S_StopMusic();
	if (!kartmode || (kartmode && leveltime > 157))
#endif
	// Start the music!
	S_Start();
