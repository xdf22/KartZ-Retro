...
#ifdef RIDERS
char gamedatafilename[64] = "srb2riders.dat";
char timeattackfolder[64] = "ridersmod";
#else
...
INT32 flashingtics = TICRATE/2;
#ifdef RIDERS
tic_t tailsflytics = 96;
#else
...
#ifdef RIDERS
const char *G_BuildAutoMapName(INT32 map)
{
	static char mapname[9] = "MAPXX"; // internal map name (wad resource name)
...
void G_BuildTiccmd(ticcmd_t *cmd, INT32 realtics)
{
...
#ifdef RIDERS
	// Weapon next and prev? Ugh, yes. :<
	// BT_TURN and cmd->turn never worked.
	if (riderslevel || kartmode)
	{
		...
		// Kartmode
	if ((kartmode && players[consoleplayer].mo &&
		(players[consoleplayer].powers[pw_introcam] < 2
		&& players[consoleplayer].speed > 0))
		// Riders
		|| (riderslevel && players[consoleplayer].mo &&
		players[consoleplayer].powers[pw_exrun] == 1))
		turnspeed = 14;
	else if (kartmode && players[consoleplayer].mo &&
		(players[consoleplayer].powers[pw_introcam] > 1
		|| players[consoleplayer].speed == 0))
		turnspeed = 0;
	else
		turnspeed = 16;
...
#ifdef RIDERS
		&& (!players[consoleplayer].spectator && gametype == GT_RACE)
#endif
...
void G_BuildTiccmd2(ticcmd_t *cmd, INT32 realtics)
{
	...
	{
		if (turnright)
			players[secondarydisplayplayer].turning = 1;
		else if (turnleft)
			players[secondarydisplayplayer].turning = -1;
		else
			players[secondarydisplayplayer].turning = 0;
	}

		// Kartmode
	if ((kartmode && players[secondarydisplayplayer].mo &&
		(players[secondarydisplayplayer].powers[pw_introcam] < 2
		&& players[secondarydisplayplayer].speed > 0))
		// Riders
		|| (riderslevel && players[secondarydisplayplayer].mo &&
		players[secondarydisplayplayer].powers[pw_exrun] == 1))
		turnspeed = 14;
	else if (kartmode && players[secondarydisplayplayer].mo &&
		(players[secondarydisplayplayer].powers[pw_introcam] > 1
		|| players[secondarydisplayplayer].speed == 0))
		turnspeed = 0;
	else
		turnspeed = 16;
...
#ifdef JTEBOTS
	JB_AddWaitingBots(consoleplayer);
#endif
}
...
boolean G_Responder(event_t *ev)
{
		...
		else
		{
			...
			while (true)
			{
				...
				if ((gametype == GT_MATCH && !cv_matchtype.value) && !(displayplayer == consoleplayer
					...
					continue;
#ifdef RIDERS
				if (gametype == GT_RACE && players[displayplayer].spectator)
					continue;
	...
	if (gameaction == ga_nothing && !singledemo &&
		...
		|| gamestate == GS_MENUSCREEN || gamestate == GS_KARTSCREEN || gamestate == GS_OPTIONSCREEN))
#else
		(demoplayback || gamestate == GS_DEMOSCREEN || gamestate == GS_TITLESCREEN))
#endif
	{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		if ((playeringame[i] || i == 0) /*&& !dedicated*/)
		{
			...
			if (kartmode)
			{
				...
				{
					cmd->forwardmove += 50/NEWTICRATERATIO;
					cmd->buttons &= ~BT_JUMP;
			...
			{
				// The way kartmode was coded BT_TAUNT was never used.
				cmd->buttons &= ~BT_TAUNT;
			}
#endif
		}
...
void G_PlayerReborn(INT32 player)
{
...
#ifdef RIDERS
	charexgear = players[player].charexgear;
	thokcolor = players[player].thokcolor;
	prefthokcolor = players[player].prefthokcolor;
	runframe = players[player].runframe;
	savecheese = players[player].cheese;
	savechip = players[player].chip;
	savebirdie = players[player].birdie;
	...
	p->maxdash = maxdash;
#ifdef RIDERS
	p->charexgear = charexgear;
	p->thokcolor = thokcolor;
	p->prefthokcolor = prefthokcolor;
	p->runframe = runframe;
	p->cheese = savecheese;
	p->chip = savechip;
	p->birdie = savebirdie;
...
#ifdef RIDERS
	p->powers[pw_airtank] = 23;
	if (p->jointime >= 1 && leveltime > 280)
		p->powers[pw_exrun] = 2;
	if (p->jointime < 1 || leveltime <= 280)
		p->powers[pw_exrun] = 0;
	...
	if ((netgame || multiplayer) && !circuitmap)
#else
	if (netgame || multiplayer)
#endif
		p->powers[pw_flashing] = flashingtics-1; // Babysitting deterrent
#ifdef RIDERS
	if (!kartmode && !p->spectator)
	{
#endif
	...
	if (kartmode && leveltime < 157 && P_IsLocalPlayer(p)) //&& !p->spectator)
		S_StopMusic();

	if (kartmode && leveltime > 157 //&& !p->spectator
	&& P_IsLocalPlayer(p) && !(splitscreen && p == &players[secondarydisplayplayer]))
	{
		if (!(mapmusic & 2048)) // TODO: Might not need this here
			mapmusic = mapheaderinfo[gamemap-1].musicslot;
	...
	//if (gametype == GT_RACE && p->spectator && P_IsLocalPlayer(p))
	//	S_ChangeMusic(mapmusic & 2047, true);
#endif
	...
	if (gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value))
	{
		...
		{
			if (p == &players[consoleplayer])
				CV_SetValue(&cv_playercolor, 13);
			else if (p == &players[secondarydisplayplayer])
				CV_SetValue(&cv_playercolor2, 13);
#else
...
#else
	if ((countdowntimeup || (!multiplayer && gametype == GT_COOP)) && !player->bot)
#endif
	{
...
#else
	if (pgametype == GT_COOP)  return TOL_RACE;
#endif
...
#ifdef RIDERS
		if (gametype == GT_RACE && nextgametype == GT_RACE)
#else
