...
#ifdef RIDERS
		// With this code, MT_ALTVIEWMAN will move up and
        // down along with you. Another cool effect!
		if (player->cutawayz == 1)
			viewz = player->mo->z + 40*FRACUNIT;
		else
#endif
		viewz = viewmobj->z + 20*FRACUNIT;
		aimingangle = player->awayviewaiming;
#ifdef RIDERS
		// This is the code that makes the camera look in the direction of the player.
		// Why keep the old one? No one uses it, and this one is much cooler, and more realistic.
		viewangle = R_PointToAngle2(viewmobj->x, viewmobj->y, player->mo->x, player->mo->y);
#else
